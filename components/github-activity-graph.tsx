  ...
  return (
    <div className="glass-panel overflow-hidden p-6 sm:p-7">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">GitHub Activity Graph</h3>
      ...
      <div className="mt-8 overflow-x-auto">
        <div className="min-w-[640px]">
          <div className="mb-3 grid grid-cols-[auto_repeat(28,minmax(0,1fr))] gap-2 text-[11px] uppercase tracking-[0.24em] text-slate-500">
            <div />
          ...
          <div className="grid grid-cols-[auto_repeat(28,minmax(0,1fr))] gap-2">
            {Array.from({ length: 7 }, (_, day) => (
              <div key={`row-${day}`} className="contents">
                <div className="flex items-center pr-2 text-[11px] uppercase tracking-[0.24em] text-slate-500">
                ...
                {Array.from({ length: weeks }, (_, week) => {
                  ...
                  return (
                    <div
                      key={`${week}-${day}`}
                      className={`aspect-square rounded-[0.35rem] border border-white/6 ${levelClasses[level]}`}
                    />
                  );
                })}
              </div>
      ...
      <div className="mt-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {levelClasses.map((levelClass) => (
            <span
              key={levelClass}
              className={`h-3 w-3 rounded-[0.25rem] border border-white/8 ${levelClass}`}
            />
          ))}
        </div>
      </div>
